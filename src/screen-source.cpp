...
const char EVENT_NAME_OPPONENT_RANK_SHOWN[] = "obsPokemonSvScreenBuilderOpponentRankShown";
...
static void screen_video_tick(void *data, float seconds)
{
	...
	cv::cvtColor(context->gameplay_bgra, gameplay_gray, cv::COLOR_BGRA2GRAY);
	cv::threshold(gameplay_gray, gameplay_binary, 128, 255, cv::THRESH_BINARY);
	SceneDetector::Scene scene =
		context->sceneDetector.detectScene(gameplay_hsv, gameplay_binary);
	...
	if (context->state == STATE_UNKNOWN) {
		if (scene == SceneDetector::SCENE_SHOW_RANK) {
			...
			context->last_state_change_ns = os_gettime_ns();
			blog(LOG_INFO, "State: UNKNOWN to ENTERING_SELECT_POKEMON");
		}
	} else if (context->state == STATE_ENTERING_SHOW_RANK) {
		context->opponentRankExtractor.extract(gameplay_binary, context->gameplay_bgra);
		...
		if (scene == SceneDetector::SCENE_SELECT_POKEMON) {
			context->state = STATE_ENTERING_SELECT_POKEMON;
			context->last_state_change_ns = os_gettime_ns();
			blog(LOG_INFO, "State: SHOW_RANK to ENTERING_SELECT_POKEMON");
		}
