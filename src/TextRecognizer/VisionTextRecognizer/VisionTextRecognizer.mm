...
cv::Mat preprocessCvMat(const cv::Mat &src)
{
    ...
    cv::Mat padded;
    cv::copyMakeBorder(resized, padded, resized.rows / 4, resized.rows / 4, resized.cols / 4, resized.cols / 4,
...
CGImageRef convertCvMatToCGImage(const cv::Mat &cvMat, OSType pixelFormatType)
{
    ...
    CVReturn retPixelBuffer = CVPixelBufferCreateWithBytes(NULL, cvMat.cols, cvMat.rows, pixelFormatType, cvMat.data,
                                                           cvMat.cols, NULL, NULL, NULL, &pixelBuffer);
    if (retPixelBuffer != kCVReturnSuccess) {
        obs_log(LOG_ERROR, "CVPixelBuffer creation failed! %d", retPixelBuffer);
        if (pixelBuffer != NULL) {
    ...
    CGImageRef image;
    OSStatus retImage = VTCreateCGImageFromCVPixelBuffer(pixelBuffer, NULL, &image);
    if (retImage != noErr) {
...
std::string recognizeByVision(CGImageRef image, NSString *langCode)
{
    ...
    VNRequestCompletionHandler completionHandler = ^(VNRequest *req, NSError *err) {
        if (err) {
            NSLog(@"%@", err);
            return;
        }
        for (VNRecognizedTextObservation *observation in req.results) {
            ...
            VNRecognizedText *recognizedText = [candidates firstObject];
            NSString *nsString = recognizedText.string;
            resultText += [nsString UTF8String];
        }
    };

    VNRecognizeTextRequest *request = [[VNRecognizeTextRequest alloc] initWithCompletionHandler:completionHandler];
...
void recognizeText(const cv::Mat imageBinary, std::function<void(std::string)> callback)
{
    dispatch_queue_t queue = dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0);

    dispatch_block_t block = ^{
        cv::Mat padded = preprocessCvMat(imageBinary);
        CGImageRef image = convertCvMatToCGImage(padded, kCVPixelFormatType_OneComponent8);
